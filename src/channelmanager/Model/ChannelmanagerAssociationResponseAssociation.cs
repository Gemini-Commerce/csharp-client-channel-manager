/*
 * Channel Manager Service
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 1.0.0
 * Contact: info@gemini-commerce.com
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = channelmanager.Client.OpenAPIDateConverter;

namespace channelmanager.Model
{
    /// <summary>
    /// ChannelmanagerAssociationResponseAssociation
    /// </summary>
    [DataContract(Name = "channelmanagerAssociationResponseAssociation")]
    public partial class ChannelmanagerAssociationResponseAssociation : IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="ChannelmanagerAssociationResponseAssociation" /> class.
        /// </summary>
        /// <param name="channelGrn">channelGrn.</param>
        /// <param name="marketGrn">marketGrn.</param>
        public ChannelmanagerAssociationResponseAssociation(string channelGrn = default(string), string marketGrn = default(string))
        {
            this.ChannelGrn = channelGrn;
            this.MarketGrn = marketGrn;
        }

        /// <summary>
        /// Gets or Sets ChannelGrn
        /// </summary>
        [DataMember(Name = "channelGrn", EmitDefaultValue = false)]
        public string ChannelGrn { get; set; }

        /// <summary>
        /// Gets or Sets MarketGrn
        /// </summary>
        [DataMember(Name = "marketGrn", EmitDefaultValue = false)]
        public string MarketGrn { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class ChannelmanagerAssociationResponseAssociation {\n");
            sb.Append("  ChannelGrn: ").Append(ChannelGrn).Append("\n");
            sb.Append("  MarketGrn: ").Append(MarketGrn).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
